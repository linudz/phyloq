...
This workflow benchmarks three strategies for defining genome–phenome
hypotheses from relative brain mass. Here, *benchmark* refers to the direct
comparison of these three predefined hypothesis-building strategies under the
...
the three approaches with prespecified result-level metrics; raw numbers of
...
- The three configurations are crossed with all alignments by Nextflow and
...
The three primary pooled configurations are:
...
are shared across the three benchmark arms in `conf/cluster.config`.
...
the three approaches and all alignments, and submits the resulting jobs in
